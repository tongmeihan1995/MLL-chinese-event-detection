...
class Data:
    def __init__(self,use_gaz): 
        ...
        self.label_alphabet_ner = Alphabet('label_ner', True)
        self.label_alphabet_general = Alphabet('label_general', True)
        self.gaz_lower = False
        self.gaz = Gazetteer(self.gaz_lower)
        self.gaz_alphabet = Alphabet('gaz')
        self.HP_fix_gaz_emb = False
        self.HP_use_gaz = use_gaz
        ...
        self.label_alphabet_size_ner = 0
        self.label_alphabet_size_general = 0
        ### hyperparameters
    ...
    def show_data_summary(self):
        ...
        #print("     NER Train instance number: %s"%(len(self.train_texts_ner)))
        #print("     NER Dev   instance number: %s"%(len(self.dev_texts_ner)))
        #print("     NER Test  instance number: %s"%(len(self.test_texts_ner)))
        #print("     NER Label alphabet size: %s"%(self.label_alphabet_size_ner))
        #print("     WIKI Train instance number: %s"%(len(self.train_texts_general)))
        #print("     WIKI Label alphabet size: %s"%(self.label_alphabet_size_general))
    ...
    def build_alphabet(self, input_file, mode):
        ...
        for idx in range(len(in_lines)):
            ...
            if len(line) > 2:
                ...
                if mode=="ace":
                    ...
                    self.label_alphabet_ner.add(label)
                elif mode=="general":
                    self.label_alphabet_general.add(label)
        ...
        self.label_alphabet_size = self.label_alphabet.size()
        self.label_alphabet_size_ner = self.label_alphabet_ner.size()
        self.label_alphabet_size_general = self.label_alphabet_general.size()
        startS = False
        startB = False
        for label,_ in self.label_alphabet.iteritems():
            if "S-" in label.upper():
        ...
        if startB:
            if startS:
                ...
                self.tagScheme = "BIO"
        # print(self.label_alphabet_size,self.label_alphabet_size_ner)
        # print(self.label_alphabet.get_content(),self.label_alphabet_ner.get_content())
    ...
    def fix_alphabet(self):
        ...
        self.label_alphabet_ner.close()
        self.label_alphabet_general.close()
    ...
    def generate_instance_with_gaz(self, input_file, name):
        ...
        if name == "train":
            ...
            self.train_texts_ner, self.train_Ids_ner = read_instance_with_gaz(input_file, self.gaz, self.word_alphabet, self.biword_alphabet, self.char_alphabet, self.gaz_alphabet,  self.label_alphabet_ner, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "dev_ner":
            self.dev_texts_ner, self.dev_Ids_ner = read_instance_with_gaz(input_file, self.gaz,self.word_alphabet, self.biword_alphabet, self.char_alphabet, self.gaz_alphabet,  self.label_alphabet_ner, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "test_ner":
            self.test_texts_ner, self.test_Ids_ner = read_instance_with_gaz(input_file, self.gaz, self.word_alphabet, self.biword_alphabet, self.char_alphabet, self.gaz_alphabet,  self.label_alphabet_ner, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "train_general":
            self.train_texts_general, self.train_Ids_general = read_instance_with_gaz(input_file, self.gaz, self.word_alphabet, self.biword_alphabet, self.char_alphabet, self.gaz_alphabet,  self.label_alphabet_general, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "raw":
